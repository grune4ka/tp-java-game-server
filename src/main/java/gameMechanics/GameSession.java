...
public class GameSession {
	...
	public GameSession() {
		...
		this.settings = (GameSettings) ResourceFactory.instanse().getResource(
				"settings/game_settings.xml");
	...
	public boolean canRemove() {
		if (System.currentTimeMillis() - this.finishTime > 1000
				&& this.finishTime != 0) {
			return true;
		} else {
			return false;
	...
	public boolean isGameEnd() {
		if (this.gamer1 != null && this.gamer2 != null) {
			if (gamer1.getPoints() >= this.settings.winCounts
					|| gamer2.getPoints() >= this.settings.winCounts) { // вынести
																		// 7ку в
																		// ресурсы.
				this.active = false;
	...
	public GameSessionSnapshot getGameSessionSnapshot() {
		...
		GameSessionSnapshot i = new GameSessionSnapshot(idGamer1, pointsGamer1,
				positionGamer1, idGamer2, pointsGamer2, positionGamer2,
				this.ballXPosition, this.ballYPosition, this.active,
				this.isGameEnd());
		return i;
	}
	...
	public void nextTick() {
		if (!this.haveFreeSlots() && !this.isGameEnd() && !this.isWait() ){
			...
			if (this.ballYPosition < 50) {
				if (this.gamer2.getBoardPosition() - 30 < this.ballXPosition
						&& this.ballXPosition < this.gamer2.getBoardPosition() + 30) {
			...
			if (this.ballYPosition > this.settings.heigthGameField - 70) {
				if ((this.gamer1.getBoardPosition() - 30 < this.ballXPosition && this.ballXPosition < this.gamer1
						.getBoardPosition() + 30)) {
					if (this.settings.vector[1] > 0) {
						this.settings.vector[1] = -this.settings.vector[1];
					}
